...
def main(expdir, recipe, computing):
    ...
    if os.path.isdir(expdir) and not os.path.isfile(os.path.join(expdir,'heldoutdata')):
        text = ''
    ...
    if os.path.exists(os.path.join(recipe,'trainfeats')):
        shutil.copyfile(os.path.join(recipe, 'trainfeats'),
                    os.path.join(expdir, 'trainfeats'))
        shutil.copyfile(os.path.join(recipe, 'traintasks'),
                    os.path.join(expdir, 'traintasks'))
    elif os.path.exists(os.path.join(expdir, 'trainfeats')):
        print 'trainfeats exists'
    else:
        ...
        for s in trainconf.get('train', 'datasections').split(' '):
            with open(os.path.join(dataconf.get(s, 'features'), 'feats')) as f:
                lines = f.readlines()
	        newlines = [s + '_' + line for line in lines]
	        fid.writelines(newlines)

            with open(dataconf.get(s, 'tasks')) as f:
                lines = f.readlines()
                newlines = [s + '_' + line for line in lines]
                tid.writelines(newlines)
