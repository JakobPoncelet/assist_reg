...
class TypeShareCoder(coder.Coder):
	...
	for t, i in self.taskindices.items():
		print('%i: task: %s' % (i+1,t))
        ...
        for argtype in structure.types:
            ...
            for t, i in sorted(self.typeindices[argtype].items(), key=operator.itemgetter(1)):
	    	print('%i: argtype: %s, option: %s' % (int(i+1), str(argtype), str(t)))
            index += len(structure.types[argtype].options)


        #save the number of labels
        self.numlabels = index
        print('numlabels: ', self.numlabels)
