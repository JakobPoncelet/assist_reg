...
class EncoderDecoder(tfmodel.TFModel):
    ...
    def model(self, inputs, seq_length, nr_spk, all_words, targets):
        ...
        return probs, tf.zeros_like(probs), tf.zeros_like(probs)

    def loss(self, targets, speakers, correct_words, probs, spklogits, wordlogits, expanded_wordfactors_present, expanded_wordfactors_absent):
        '''compute the loss
        ...
        return loss, tf.zeros_like(loss), tf.zeros_like(loss), tf.zeros_like(loss)
